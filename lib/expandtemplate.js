function expandtemplate(options,callback) {
	...
	function parseISO8601Duration(Duration) {
		if (Duration.match("PT")) {
			var sign    = parseInt(Duration.replace(/(.*)PT.*/,"$11"));
			...
			var hours   = sign*parseInt(Duration.replace(/.*?([0-9]*)H.*/,"$1"));
			var minutes = sign*parseInt(Duration.replace(/.*?([0-9]*)M.*/,"$1"));
			var seconds = sign*parseInt(Duration.replace(/.*?([0-9]*)S.*/,"$1"));
			if (debug) console.log("sign: " + sign);
			if (debug) console.log("days: " + days);
			if (debug) console.log("hours: " + hours);
			Duration = new Date(new Date().toUTCString().substr(0,16)).add({years:years,months:months,days:days,hours:hours,minutes:minutes,seconds:seconds});
			if (debug) console.log(Duration);
			if (debug) console.log(parseInt(Duration.getDay()));
			return Duration.getFullYear() + "-" + sprintf("%02d",parseInt(Duration.getMonth())+1) + "-" + sprintf("%02d",parseInt(Duration.getDate()));
		} else {
			return Duration;
		}

	}
	Start = parseISO8601Duration(Start);
	Stop = parseISO8601Duration(Stop);
	...
	if (type == "strftime") {
		...
		if (debug) console.log("START_dateoff = " + START_dateoff);
		if (debug) console.log("STOP_date     = " + STOP_date);
